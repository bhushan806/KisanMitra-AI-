...
def authenticate_and_initialize():
    ...
    if not gee_project_id:
        logger.error(
            "GEE_PROJECT_ID is missing from the environment. Please add it to backend/.env"
        )
        sys.exit(1)
    ...
    try:
        ...
        logger.info(
            f"Successfully initialized Earth Engine with project: {gee_project_id}"
        )
    except Exception as e:
        logger.error(f"Failed to initialize Earth Engine: {e}")
        logger.warning(
            "Attempting to authenticate. This may require interactive login in the terminal."
        )
        try:
            ee.Authenticate()
            ee.Initialize(project=gee_project_id)
...
def get_ndvi_for_district(district_name, coords, start_date, end_date):
    ...
    filtered = (
        s2.filterBounds(region).filterDate(start_date, end_date)
    ...
    try:
        ...
        if count == 0:
            logger.warning(
                f"No valid Sentinel-2 images found for {district_name} between {start_date} and {end_date}."
            )
            return None
    except Exception as e:
...
def ingest_satellite_data(start_date_str, end_date_str=None):
    ...
    logger.info(
        f"Ingesting Sentinel-2 satellite data for period: {start_date_str} to {end_date_str}"
    )
    ...
    for district, coords in DISTRICTS.items():
        ...
        ndvi_value = get_ndvi_for_district(
            district, coords, start_date_str, end_date_str
        )
        ...
        if ndvi_value is not None:
            ...
            ingested_records += 1
            logger.info(
                f"Successfully saved NDVI={ndvi_value:.4f} for {district} on {start_date_str}."
            )
        else:
            logger.warning(
                f"Could not retrieve NDVI for {district} on {start_date_str}."
            )

    logger.info(
        f"Satellite ingestion completed. Inserted/Updated {ingested_records} records."
    )
