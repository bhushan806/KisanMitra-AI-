...
async def get_historical_prices(commodity: str):
    ...
    try:
        ...
        cursor = collection.find({"commodity": commodity}).sort("date", pymongo.ASCENDING)
...
async def predict_price(
    ...
    if not hist_prices:
        raise HTTPException(status_code=404, detail=f"No data found for {commodity}. Run training first.")

    forecast_data = generate_forecast(commodity, mandi, horizon, last_features, hist_prices)

    if not forecast_data:
        raise HTTPException(status_code=500, detail="Failed to generate forecast. Models might not be trained.")

    # Check Alerts
    alert_level, trigger_price, avg_price = check_alerts(forecast_data["forecast"], hist_prices)
    ...
    if dq_doc:
        ...
        forecast_data["data_quality"] = {"real_records": 0, "synthetic_records": 0, "real_percentage": "0.0%"}
